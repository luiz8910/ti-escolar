...
@sem_minio
async def test_listar_chaves_respeita_prefixo_idade_e_limite():
    """O contrato do varredor de órfãos. Três coisas que o `ListObjectsV2` **não** faz
    sozinho e por isso precisam de teste contra o serviço real: ele não filtra por
    `LastModified` (só devolve o campo), pagina por conta própria, e o `MaxKeys` cru
    contaria objeto novo — a varredura acharia que não há lixo."""
    from datetime import datetime, timedelta, timezone

    s = _storage()
    tenant = uuid.uuid4()
    doc = f"doc/{tenant}/2026/08/{uuid.uuid4().hex}"
    impressao = f"impressao/{tenant}/2026/08/{uuid.uuid4().hex}"
    await s.guardar(chave=doc, conteudo=b"a", mime="text/plain")
    await s.guardar(chave=impressao, conteudo=b"b", mime="text/plain")

    futuro = datetime.now(timezone.utc) + timedelta(hours=1)
    passado = datetime.now(timezone.utc) - timedelta(hours=1)

    # o prefixo separa as finalidades, que é o que as regras de lifecycle enxergam
    assert await s.listar_chaves(
        prefixo=f"doc/{tenant}/", criado_antes_de=futuro
    ) == [doc]
    assert await s.listar_chaves(
        prefixo=f"impressao/{tenant}/", criado_antes_de=futuro
    ) == [impressao]

    # o corte por idade é o que impede a varredura de apagar um upload em curso
    assert (
        await s.listar_chaves(prefixo=f"doc/{tenant}/", criado_antes_de=passado) == []
    )

    # e o limite conta o que passou pelo filtro, não o que o S3 devolveu
    assert (
        len(
            await s.listar_chaves(
                prefixo=f"doc/{tenant}", criado_antes_de=futuro, limite=1
            )
        )
        == 1
    )

    await s.remover(chave=doc)
    await s.remover(chave=impressao)


@sem_minio
async def test_listar_chaves_em_prefixo_vazio_nao_explode():
    """Prefixo sem nenhum objeto devolve lista vazia, não exceção: `Contents` simplesmente
    não vem na resposta do S3, e ler a chave direto daria `KeyError`."""
    from datetime import datetime, timedelta, timezone

    vazio = await _storage().listar_chaves(
        prefixo=f"doc/{uuid.uuid4()}/",
        criado_antes_de=datetime.now(timezone.utc) + timedelta(hours=1),
    )
    assert vazio == []
...
def test_finalidade_e_tenant_sao_parametros_separados():
    """A finalidade decide **qual regra de lifecycle** apaga o objeto, então ela não pode
    depender de o chamador montar a string certa: `impressao/` cai na regra de 180 dias e
    `doc/` na de 395. Um prefixo montado à mão já produziu `doc/doc/{tenant}/…`."""
    from app.infrastructure.storage import nova_chave

    tenant = uuid.uuid4()
    assert nova_chave(tenant, "impressao").startswith(f"impressao/{tenant}/")
    assert nova_chave(tenant, "foto").startswith(f"foto/{tenant}/")
    # nada de finalidade duplicada no caminho
    assert nova_chave(tenant, "doc").count("doc/") == 1
