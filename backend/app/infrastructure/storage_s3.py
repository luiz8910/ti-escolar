...
class S3ArquivoStorage:
    ...
    **A atomicidade acabou, e isso é do desenho, não um descuido.** O
    ...
    async def listar_chaves(
        ...
        """Chaves sob ``prefixo`` mais antigas que ``criado_antes_de`` (varredor de órfãos).

        O corte por idade é feito **aqui e não no S3**: o `ListObjectsV2` não filtra por
        `LastModified`, só devolve o campo. Por isso a paginação para no `limite` já
        filtrado — pedir `MaxKeys=limite` cru poderia devolver mil objetos novos e nenhum
        órfão, e a varredura concluiria que não há lixo.

        `LastModified` vem do S3 em UTC e é o carimbo do **objeto**, não do metadado; um
        objeto regravado rejuvenesce, o que é conservador do lado certo — na dúvida, a
        varredura não apaga.
        """
        ...
        def _listar() -> list[str]:
            ...
            for pagina in paginador.paginate(
                Bucket=self._bucket, Prefix=prefixo or ""
            ):
                for objeto in pagina.get("Contents", []):
                    if objeto["LastModified"] < criado_antes_de:
                        achadas.append(objeto["Key"])
